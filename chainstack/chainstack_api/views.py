...
def generate_token(request):
    ...
    if username and password:
        ...
        if user and user[0].check_password(password):
            ...
            return JsonResponse(
                {"token": token.key}, safe=False, status=status.HTTP_201_CREATED
            )
        else:
            return JsonResponse(
        ...
        return JsonResponse(
            {"message": "no user name or password in request"},
            safe=False,
...
def create_news_item(request):
    ...
    try:
        if log_and_validate_request(request.user):
            ...
            return JsonResponse(
                {"message": "Rate limit exceeded or invalid user"},
        ...
        return JsonResponse(
            {"message": "Error creating news item please try again later"},
...
def log_and_validate_request(user, rate_limit=0):
    ...
    if user_object:
        ...
        if request_tracker:
            ...
            if (
                request_tracker.current_request_count >= request_tracker.request_limit
                and request_tracker.request_limit != 0
            ):
                return False
            ...
            user = user[0]
            request_tracker = APIRequestTracker.objects.create(
                **{"user": user, "current_request_count": 1}
            )
            request_tracker.save()
            return True
    else:
    ...
    try:
        if log_and_validate_request(request.user):
            ...
            if news_id:
                ...
                if item and check_superuser(
                    request.user
                ):  # delete directly if superuser
                    item.delete()
                    return JsonResponse(
                        {"message": "News Item deleted"}, status=status.HTTP_200_OK
                    )
                if (
                    item and item.created_by == request.user
                ):  # user should delete only resources they create
                    item.delete()
                    return JsonResponse(
                        {"message": "News Item deleted"}, status=status.HTTP_200_OK
                    )
                    ...
                    return JsonResponse(
                        {
                            "message": "News Item does not exist in database or you dont have permission to delete"
                        },
                ...
                return JsonResponse(
                    {"message": "No news_id in request data"},
                    status=status.HTTP_417_EXPECTATION_FAILED,
                )
        else:
            return JsonResponse(
        ...
        return JsonResponse(
            {
                "message": "Error deleting news item please try again later",
...
def read_news(request):
    ...
    try:
        ...
        return JsonResponse(
            {"message": "Error creating listing news items please try again later"},
            status=status.HTTP_500_INTERNAL_SERVER_ERROR,
        )
    ...
    try:
        if check_superuser(request.user):
            ...
            if username and password and email:
                ...
                user_model.save()
                return JsonResponse(
                    {"message": "User Created", "user_details": user_dict},
                    status=status.HTTP_201_CREATED,
                )
                ...
                return JsonResponse(
                    {
                        "message": "Username,password and email are manadatory parameters"
                    },
                    status=status.HTTP_417_EXPECTATION_FAILED,
                )
        else:
    ...
    try:
        if check_superuser(request.user):
            ...
            if username:
                ...
                return JsonResponse(
                    {"message": "No username in request parameter"},
                    status=status.HTTP_417_EXPECTATION_FAILED,
                )
        else:
            return JsonResponse(
...
def list_users(request):
    ...
    try:
        if check_superuser(request.user):
            ...
            user_list = (
                User.objects.all()
                .values("username", "email", "is_superuser", "date_joined")
                .order_by("-username")
            )
            if user_list:
            ...
            return JsonResponse(
                {"message": "No users in db"}, status=status.HTTP_404_NOT_FOUND
            )
        else:
            return JsonResponse(
                {"message": "only admin can list users"},
                status=status.HTTP_401_UNAUTHORIZED,
            )
        ...
        return JsonResponse(
            {"message": "Error listing users please try again later"},
            status=status.HTTP_500_INTERNAL_SERVER_ERROR,
        )
...
def set_rate_limit_for_user(request):
    ...
    try:
        if check_superuser(request.user):
            ...
            if user and User.objects.filter(username=user):
                ...
                if user_model and user_model[0].is_superuser:
                    return JsonResponse(
                        {"message": "superuser has no limit"},
                        safe=False,
                        status=status.HTTP_417_EXPECTATION_FAILED,
                    )
                ...
                if request_tracker:
                    ...
                    request_tracker = APIRequestTracker.objects.create(
                        **{"user": user_model[0], "request_limit": request_limit}
                    )
                    request_tracker.save()
                    return JsonResponse(
                        {"message": "Rate limit set"},
            ...
            return JsonResponse(
                {"message": "only admin can set rate limit"},
                status=status.HTTP_401_UNAUTHORIZED,
            )
        ...
        return JsonResponse(
            {"message": "Error setting quota please try again later"},
            status=status.HTTP_500_INTERNAL_SERVER_ERROR,
        )
...
def create_admin(request):
    ...
    try:
        ...
        if username and password and email:
            ...
            user_model.save()
            return JsonResponse(
                {"message": "Platform Admin Created", "user_details": user_dict},
                status=status.HTTP_201_CREATED,
            )
        else:
            return JsonResponse(
        ...
        return JsonResponse(
            {"message": "Error creating platform admin"},
            status=status.HTTP_500_INTERNAL_SERVER_ERROR,
        )
...
def log_error(error, log_view):
    ...
    error_log = ErrorLog.objects.create(
        **{"log": error, "log_view": log_view, "log_id": generate_id()}
    )
    error_log.save()
